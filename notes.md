...
RAG(검색 기반 생성), LangChain 파이프라인, LangGraph 상태 머신을 활용해 MSP 운영 정책 문서에 특화된 챗봇을 개발하며 배운 내용을 정리한 노트.
...
에이전트 모드(k8s 제어)와 일반 RAG 모드를 단일 파이프라인으로 통합.
모드 전환 체크박스 제거 → 모든 질문이 LangGraph 파이프라인을 거치며, `classify_intent` 노드가 자동으로 라우팅.

"nginx 재시작해줘" 같은 k8s 명령과 "VM 장애 대응 절차 알려줘" 같은 RAG 질문 모두 단일 엔드포인트(`/chat-graph-stream`)로 처리.
...
[classify_intent]  ← LLM이 "k8s 명령인가? question인가?" 판단
 ...
 └── question → [check_relevance]
                  ├── yes → [retrieve_and_answer] → END
                  └── no  → [reject] (LLM 자유 응답) → END
```
...
| 노드 | 역할 |
|---|---|
| `classify_intent` | k8s 명령 vs 일반 질문 분류 |
| `parse_k8s_command` | action/target/namespace JSON 추출 |
| `execute_k8s` | kubectl 실행 (k8s_tools.py) |
| `check_relevance` | MSP 운영 관련 여부 판단 |
| `retrieve_and_answer` | 벡터DB 검색 후 매뉴얼 기반 답변 |
| `reject` | 비MSP 질문에 LLM이 자유롭게 응답 (인사 등) |

### reject 노드 — LLM 제약 응답

이전에는 고정 문자열 반환이었으나, 인사말("안녕")에 딱딱하게 반응하는 문제로 LLM 응답으로 변경.
할루시네이션 방지를 위해 프롬프트로 제약:
...
- "안녕" → "안녕하세요! 무엇을 도와드릴까요?" ✓
- "삼성 주가 알려줘" → "해당 내용은 답변이 어렵습니다" ✓

### classify_intent 파싱 — 엄격한 매칭

LLM이 "이것은 k8s 명령이 아닙니다"처럼 설명형으로 답하면 문장 안의 "k8s"가 오탐지됨.
구두점만 제거 후 정확히 "k8s"인지 비교:
...
### on_chain_end AIMessage 오류

LangGraph에서 `on_chain_end` 이벤트가 노드당 2번 발생:
1. 노드 내부 LLM 체인 종료 → output = `AIMessage`
2. 노드 자체 종료 → output = `GraphState` dict

`output.get()`은 dict에만 동작하므로 `isinstance(output, dict)` 체크 필수:
...
_to_md_table(text)           # kubectl 출력을 마크다운 테이블로 변환 (위치 기반 파싱)
```

네임스페이스를 명시하지 않으면 `find_deployment()`로 전 네임스페이스에서 자동 탐색.
kubectl 출력은 위치 기반 파싱으로 마크다운 테이블 변환 (멀티워드 헤더 지원).

### 추천 질문

모드 통합 후 `/suggestions` API 단일 경로 사용 (통계 기반 + 사전 정의).
k8s 명령도 `mode='chat'`으로 저장되어 통계에 반영됨.
